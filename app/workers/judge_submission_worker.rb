...
class JudgeSubmissionWorker < ApplicationWorker
  ...
  def self.judge(submission, queue: nil, delay: 0)
    ...
    if user.competing? && Pundit.policy(user.submission).show?
      if rejudging # first time judging, earlier submissions get better priority (so other people get a chance to have their stuff judged)
        ...
        priority = 20 - min(prev,10)
      else # re-judging, later submissions are more important
