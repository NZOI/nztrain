...
class JudgeSubmissionWorker < ApplicationWorker
  ...
  def self.judge(submission, queue: nil, delay: 0)
    ...
    put({id: submission.id}, queue: queue, delay: delay, priority: priority)
  end

  def self.perform(job)
    result = new.perform(job.data["id"])
    job.complete
  end
  ...
  def judge
    ...
    setup_judging do
      ...
      if problem.evaluator.nil?
        ...
        eval_command = if problem.evaluator.language.nil?
          "./#{EvalFileName}"
        else
          problem.evaluator.language.run_command(EvalFileName)
        end
  ...
  def grade_submission(graded_sets, preres = nil)
    ...
    if result["status"] == 0
      ...
      result["score"] = if maximum_points == 0
        0
      else
        preres.nil? || preres["evaluation"] != 0 ? (result["evaluation"] * 100).floor.to_i : 0
      end
  ...
  def grade_compile_error(compiled)
    ...
    when 0
      {"status" => 1} # pending
    when 1
      {"status" => 0, "evaluation" => 0.0, "score" => 0, "points" => 0.to_d}
    else
      {"status" => 2} # errored
    end
  end
