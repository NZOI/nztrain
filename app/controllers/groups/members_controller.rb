class Groups::MembersController < ApplicationController
  ...
  def accept
    load_group_request
    case @request_type
    when :invitation then authorize @request, :accept?
  ...
  def reject
    load_group_request
    case @request_type
    when :invitation, :join_request then authorize @group, :reject?
    else raise Pundit::NotAuthorizedError
    end
    @request.reject!
  ...
  def cancel
    load_group_request
    case @request_type
    when :invitation then policy(@group).reject? or authorize @request, :cancel?
    when :join_request then authorize @group, :cancel?
    else raise Pundit::NotAuthorizedError
    end
    @request.cancel!
    if @request_type == :invitation
