class Groups::FilesController < ApplicationController
  ...
  def index
    ...
    @files = @group.file_attachments.order(:filepath)
  ...
  def show
    @group = Group.find(params[:group_id])
    authorize @group, :access?
    @file = @group.file_attachments.find(params[:id])
  ...
  def create
    @group = Group.find(params[:group_id])
    authorize @group, :update?
    @new_file = @group.file_attachments.build(group_file_attachment_params)
    if @new_file.save
  ...
  def destroy
    ...
    authorize @group, :update?
    @file = @group.file_attachments.find(params[:id])
    if @group.file_attachments.destroy(@file)
