class Group < ApplicationRecord
  ...
  def join(current_user)
    if members.exists?(current_user.id)
      false
    else
      members.push(current_user)
      # consider any pending invitations/applications accepted
      invitations.pending.where(target_id: current_user.id).each(&:accept!)
      join_requests.pending.where(subject_id: current_user.id).each(&:accept!)
      true
    end
  end
  ...
  def apply!(current_user, user = nil)
    ...
    Request.create(requester: current_user, subject: user, verb: :join, target: self, requestee: owner)
  end
