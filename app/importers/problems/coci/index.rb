  ...
  module COCI
    module Index
      ...
      def update
        ...
        candidate_links = rootpage.links_with(href: /^(#{SOURCE})?archive\/[[:digit:]]{4}.[[:digit:]]{4}\/?$/o) + rootpage.links_with(text: /^COCI [[:digit:]]{4}.[[:digit:]]{4}$/).uniq { |link| link.href }
      ...
      def downloaded?(vn, cn)
        ...
        %i[tasks testdata].each do |attr|
          ...
          return false unless File.exist?(File.expand_path(issue[attr][:local], DATAPATH))
        end
        true
      end
      ...
      def index_page(page, title = nil)
        if title.nil?
          ...
          title = if years.size > 2 && years.min + 1 == years.max
            "COCI #{years.min}/#{years.max}"
          else
            page.root.xpath(".//*[not(ancestor::a) and series_match(.)]", COCINameMatcher).first.text
          end
        end
      ...
      def set_problem_set_id(vid, cid, id)
        ...
        elsif ProblemSet.find(id)
          issue[:problem_set_id] = id
        end
        save
      end
      ...
      def set_problem_id(vid, cid, pid, id)
        ...
        elsif Problem.find(id)
          issue[:problems][pid][:problem_id] = id
        end
        save
      end
