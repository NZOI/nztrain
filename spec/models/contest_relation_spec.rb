...
describe ContestRelation do
  ...
  before(:all) do
    ...
    @relation = Factory.create(:contest_relation, :contest => @contest, :user => users(:user))
  end
  after(:all) do
    @relation.destroy
    @contest.destroy
  ...
  it "updates finish_at when contest changes" do
    @anothercontest = Factory.build(:contest, :start_time => @relation.started_at.advance(:hours => -1), :end_time => @relation.started_at.advance(:hours => 1))
